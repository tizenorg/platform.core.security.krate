...
char *PasswordSetupPage::getLabel(const std::string& name, const std::string& part)
{
	return nullptr;
}

Widget *PasswordSetupPage::getWidget(Genlist* parent, const std::string& name, const std::string& part)
{
	if ((name == "password-input")) {
		entryLayout = new Layout(parent, EDJE_FILE, "security_layout");
		entryLayout->setText("Enter Password", "title");

		entry = new Entry(entryLayout, Entry::SetSingleLine | Entry::SetPassword);
		entry->setFocus(true);
		entry->onChanged = [this]() {
			const std::string entry_data = entry->get();
			if (entry_data.length() > 1) {
				nextButton->emitSignal("confirmed", "next-button");
			}
		};
...
void PasswordSetupPage::createPartContents()
{
	...
	nextButton->addSignalHandler("confirmed", "next-button",
