...
//static int security_passphrase_mode = 0;
...
void PasswordSetupPage::createPartContents()
{
	class ContentProvider : public GenlistContentProvider {
	public:
		ContentProvider(void *data) : userData(data) {}
			...
			if ((name == "password-input")) {
				...
				entry->onChanged = [this]() {
					...
					if (entry_data.length() > 1) {
						reinterpret_cast<Widget*>(userData)->signalEmit("confirmed", "next-button");
					}
				};
	...
	Genlist *genlist = new Genlist(this, new ContentProvider(nextButton));
	...
	nextButton->addSignalHandler("confirmed", "next-button", [this](const std::string& emission, const std::string& source) {
