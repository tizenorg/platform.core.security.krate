...
public:
	...
	virtual void setContent(Widget* content) {
		elm_object_content_set(nativeObject, content->getObject());
	}

	virtual void setContent(Widget* content, const std::string& part) {
		elm_object_part_content_set(nativeObject, part.c_str(), content->getObject());
	}
protected:
