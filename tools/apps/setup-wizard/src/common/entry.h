...
public:
	Entry(Widget *parent)
	{
		...
		addSignalHandler("edje,focuse,out", "edje", [this](const std::string& emission, const std::string& source) {
			onUnfocused();
		});
		addSignalHandler("elm,changed", "elm.text", [this](const std::string& emission, const std::string& source) {
			onChanged();
		});
	}
	...
	void setReturnKeyDisabled(bool disabled)
	{
		elm_entry_input_panel_return_key_disabled_set(nativeObject, disabled);
	}
