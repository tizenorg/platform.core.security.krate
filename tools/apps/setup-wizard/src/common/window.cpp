...
void Window::addSignalHandler(const std::string& emission, const std::string& source, SignalHandler&& handler)
{
	SignalInfo *info = new SignalInfo();
	info->klass = this;
	info->handler = std::move(handler);

	elm_object_signal_callback_add(Widget::object(conformant), emission.c_str(), source.c_str(), signalHandlerDispatcher, reinterpret_cast<void *>(info));
}

void Window::removeSignalHandler(const std::string& emission, const std::string& source)
{
	void *data = elm_object_signal_callback_del(*conformant, emission.c_str(), source.c_str(), signalHandlerDispatcher);
	if (data != NULL) {
		delete reinterpret_cast<SignalInfo *>(data);
	}
}

void Window::emitSignal(const std::string& emission, const std::string& source)
{
	elm_object_signal_emit(*conformant, emission.c_str(), source.c_str());
}
