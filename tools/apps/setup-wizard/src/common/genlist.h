...
public:
	virtual Widget* getContent(Genlist* parent, const std::string& name, const std::string& part) = 0;
};
...
public:
	Genlist(Widget *parent) :
		labelProvider(nullptr), contentProvider(nullptr)
	{
	...
	GenlistItem append(const std::string& name, const std::string& style, Elm_Object_Select_Mode mode)
	{
	...
	void setLabelProvider(GenlistLabelProvider* provider)
	{
...
private:
	...
	{
		GenlistItemData* self = reinterpret_cast<GenlistItemData *>(data);
		Widget* widget = self->getContent(self->genlist, self->name, part);
...
private:
	struct GenlistItemData {
		...
		{
			GenlistLabelProvider *labelProvider = genlist->getLabelProvider();
			if (labelProvider) {
				return labelProvider->getLabel(name, part);
			}
		...
		Widget* getContent(Genlist* parent, const std::string& name, const std::string& part)
		{
			...
			if (contentProvider) {
				return contentProvider->getContent(parent, name, part);
			}
	...
	GenlistLabelProvider* labelProvider;
