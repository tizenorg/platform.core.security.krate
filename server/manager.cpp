...
const std::string KRATE_MANIFEST_DIR = CONF_PATH "/krate/";
...
void initializeCreatedKrateList() {
	try {
		runtime::DirectoryIterator iter(KRATE_MANIFEST_DIR), end;
...
int Manager::createKrate(const std::string& name, const std::string& manifest)
{
	auto provisioningWorker = [name, manifest, this]() {
		...
		try {
			...
			manifestFile->write(KRATE_MANIFEST_DIR + name + ".xml", "UTF-8", true);
